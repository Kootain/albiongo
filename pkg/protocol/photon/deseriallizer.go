...
type PhotonMap map[interface{}]interface{}

func (m PhotonMap) MarshalJSON() ([]byte, error) {
...
func isComparable(v interface{}) bool {
	...
	case nil, bool, byte, int16, int32, int64, float32, float64, string:
		return true
	case []interface{}, []bool, []byte, []int16, []int32, []int64, []float32, []float64, []string,
		PhotonMap, map[interface{}]interface{}, map[byte]interface{}:
		return false
	}
...
func deserializeDictionary(buf *bytes.Buffer) PhotonMap {
...
func deserializeHashtable(buf *bytes.Buffer) PhotonMap {
	return deserializeDictionary(buf)
}
...
func ReadParameterTableWithCount(count int, buf *bytes.Buffer) map[byte]interface{} {
	if count < 0 || count > maxArraySize || count > buf.Len() {
...
func ReadParameterTable(buf *bytes.Buffer) map[byte]interface{} {
	count := int(readCount(buf))
	return ReadParameterTableWithCount(count, buf)
}
